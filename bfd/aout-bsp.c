/* BFD back-end for SGI O2 VICE BSP a.out binaries.
   Copyright 2002 Free Software Foundation, Inc.
   Authore: Ilya Volynets <ilya@theIlya.com>
...
/* BFD backend for BSP
   ...
   This file was hacked up by looking hard at the pdp11 back end.
...
#define	DEFAULT_ARCH	bfd_arch_bsp
#define	DEFAULT_MID 	M_BSP

/* Do not "beautify" the CONCAT* macro args.  Traditional C will not
   remove whitespace added here, and thus will fail to concatenate
   the tokens.  */
#define MY(OP) CONCAT2 (bsp_aout_,OP)

/* This needs to start with a.out so GDB knows it is an a.out variant.  */
#define TARGETNAME "a.out-bsp"
...
#define external_exec bsp_external_exec
struct bsp_external_exec
  {
...
#define external_nlist bsp_external_nlist
struct bsp_external_nlist
{
...
#define WRITE_HEADERS(abfd, execp) bsp_aout_write_headers (abfd, execp)
...
const bfd_target MY(vec) =
{
  ...
  BFD_ENDIAN_BIG,		/* target headers byte order (little) */
...
struct bsp_aout_reloc_external
{
...
static int bsp_aout_write_headers PARAMS ((bfd *, struct internal_exec *));
void bsp_aout_swap_reloc_out PARAMS  ((bfd *, arelent *, struct bsp_aout_reloc_external *));
void bsp_aout_swap_reloc_in
PARAMS ((bfd *, struct bsp_aout_reloc_external *, arelent *,
...
reloc_howto_type howto_table_bsp[] =
{
...
static int
bsp_aout_write_headers (abfd, execp)
     bfd *abfd;
...
void
bsp_aout_swap_reloc_out (abfd, g, natptr)
     bfd *abfd;
     arelent *g;
     register struct bsp_aout_reloc_external *natptr;
{
  int r_index;
  int r_pcrel;
  int reloc_entry;
...
void
bsp_aout_swap_reloc_in (abfd, bytes, cache_ptr, offset,
			  symbols, symcount)
     bfd *abfd;
     struct bsp_aout_reloc_external *bytes;
     arelent *cache_ptr;
     ...
     bfd_size_type symcount;
{
  ...
  cache_ptr->howto = howto_table_bsp + (r_pcrel ? 1 : 0);
...
NAME(aout,slurp_reloc_table) (abfd, asect, symbols)
     ...
     asymbol **symbols;
{
  struct bsp_aout_reloc_external *rptr;
  bfd_size_type count;
  ...
  rptr = (struct bsp_aout_reloc_external *) relocs;
  for (counter = 0;
       counter < count;
      ...
      while (GET_WORD (abfd, (PTR)rptr) == 0)
	{
      ...
      bsp_aout_swap_reloc_in (abfd, rptr, cache_ptr,
  ...
  if (generic != NULL)
    {
      while (count > 0)
	{
	  struct bsp_aout_reloc_external *r;

	  r = (struct bsp_aout_reloc_external *)
	    (native + (*generic)->address);
	  bsp_aout_swap_reloc_out (abfd, *generic, r);
	  count--;
	  generic++;
	}
    }
...
/* This is stupid.  This function should be a bfd_boolean predicate */
long
...
aout_link_add_symbols (abfd, info)
     ...
     struct bfd_link_info *info;
{
  bfd_boolean (*add_one_symbol) PARAMS ((struct bfd_link_info *, bfd *,
				     const char *, flagword, asection *,
				     bfd_vma, const char *, bfd_boolean,
				     bfd_boolean,
...
static bfd_boolean bsp_aout_link_input_section
  PARAMS ((struct aout_final_link_info *finfo,
	   bfd *input_bfd,
	   asection *input_section,
	   struct bsp_aout_reloc_external *relocs,
	   bfd_size_type rel_size,
	   bfd_byte *contents));
     ...
     bfd_size_type rel_size;
{
  ...
  if (! bsp_aout_link_input_section (finfo, input_bfd, input_section,
				       (struct bsp_aout_reloc_external *) relocs,
...
static bfd_boolean
bsp_aout_link_input_section (finfo, input_bfd, input_section, relocs,
			       rel_size, contents)
     struct aout_final_link_info *finfo;
     bfd *input_bfd;
     asection *input_section;
     ...
     bfd_byte *contents;
{
  bfd_boolean (*check_dynamic_reloc) PARAMS ((struct bfd_link_info *,
					  bfd *, asection *,
					  struct aout_link_hash_entry *,
					  PTR, bfd_byte *, bfd_boolean *,
					  bfd_vma *));
  bfd *output_bfd;
  ...
  register struct bsp_aout_reloc_external *rel;
  struct bsp_aout_reloc_external *rel_end;
  ...
  rel = relocs;
  rel_end = (struct bsp_aout_reloc_external *)(((char *)rel) + rel_size);
  for (; rel < rel_end; rel += RELOC_SIZE)
	...
	howto_idx = r_pcrel;
	BFD_ASSERT (howto_idx < TABLE_SIZE (howto_table_bsp));
	howto = howto_table_bsp + howto_idx;
      }
     ...
     struct bfd_link_order *p;
{
...
#else
  {
    ...
    fprintf (stderr, "TODO: line %d in bfd/bsp.c\n", __LINE__);
